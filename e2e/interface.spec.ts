...
test("la priorité d'un article se cycle par clic, sans changer l'ordre des articles ni des catégories", async ({ page }) => {
  await page.goto("/");
  await page.click("#create-form button[type=submit]");
  await page.waitForURL(/\/l\//);
  await expect(page.locator(".conn-dot")).toHaveClass(/online/, { timeout: 10_000 });
  ...
  await page.selectOption("#add-category", { label: "Bricolage" });
  for (const name of ["Marteau", "Pommes", "Poires"]) {
    await page.fill("#add-input", name);
    await page.click(".add-submit");
  }
  await page.selectOption("#add-category", { label: "Papeterie" });
  ...
  // Par défaut, tous les articles sont en priorité Normale.
  await expect(page.locator(".category-name")).toHaveText(["Bricolage", "Papeterie"]);
  await expect(page.locator(".item-name")).toHaveText(["Marteau", "Pommes", "Poires", "Stylo"]);
  const poiresItem = page.locator(".item", { has: page.locator(".item-name", { hasText: "Poires" }) });
  const poiresPriority = poiresItem.locator(".item-priority");
  await expect(poiresItem).toHaveAttribute("data-priority", "1");
  await expect(poiresPriority).toHaveAttribute("aria-label", /Normale/);

  // Un clic fait passer Normale -> Haute : ni l'article ni sa catégorie ne
  // bougent, seule son apparence change (voir aussi "priorité plus visible").
  await poiresPriority.click();
  await expect(poiresItem).toHaveAttribute("data-priority", "2");
  await expect(poiresPriority).toHaveAttribute("aria-label", /Haute/);
  await expect(page.locator(".item-name")).toHaveText(["Marteau", "Pommes", "Poires", "Stylo"]);
  await expect(page.locator(".category-name")).toHaveText(["Bricolage", "Papeterie"]);
  ...
  await expect(poiresPriority).toHaveAttribute("aria-label", /Basse/);
  await expect(page.locator(".item-name")).toHaveText(["Marteau", "Pommes", "Poires", "Stylo"]);
  await expect(page.locator(".category-name")).toHaveText(["Bricolage", "Papeterie"]);
